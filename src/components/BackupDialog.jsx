...
/**
 * Dialog per visualizzare e ripristinare backup del database documento
 *
 * Props:
 * - documentId (string): UUID del documento
 * - currentDbPath (string): Percorso del database corrente
 * - onClose (function): Callback per chiudere il dialog
 * - onRestoreSuccess (function): Callback chiamato dopo ripristino riuscito
 */
function BackupDialog({ documentId, currentDbPath, onClose, onRestoreSuccess }) {
  ...
  const loadBackups = async () => {
    ...
    try {
      const backupList = await window.electronAPI.backupList(documentId);
      setBackups(backupList);
    } catch (err) {
      console.error('Error loading backups:', err);
      setError('Errore durante il caricamento dei backup');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRestore = async (backup) => {
    ...
    const confirmed = await showConfirmDialog({
      ...
      message: `Vuoi ripristinare il backup del <b>${formatDate(backup.timestamp)}</b>?`,
      detail: `Questa azione sovrascriverà il database corrente.\nSarà creato automaticamente un backup dello stato attuale prima del ripristino.`,
          ...
          {!loading && !error && backups.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm mt-2">
                I backup vengono creati automaticamente all'apertura del documento
              </p>
            </div>
          )}
